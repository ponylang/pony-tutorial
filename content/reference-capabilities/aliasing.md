...
__What about aliasing other stuff?__ For both `iso` and `trn`, the guarantees impose aliases to give up on some ability (reading and writing for `iso`, writing for `trn`). For the other capabilities `ref`, `val`, `box` and `tag`, aliases allow for the same operations, so a reference can just be aliased as itself.
